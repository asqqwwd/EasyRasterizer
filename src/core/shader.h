...
namespace Core
{
    ...
    namespace GouraudShader
    {
        VertexOutput vert(const VertexInput &vi, const Attribute &attribute, const Uniform &uniform)
        {
            ...
            Vector3f diffuse = attribute.light_color * sampling(uniform.albedo, vi.UV) * attribute.light_intensity * std::max(0.f, Utils::dot_product_3D(world_normal, -1 * attribute.world_light_dir));
    ...
    namespace PhongShader
    {
        ...
        Vector4i frag(FragmentInput fi, const Attribute &attribute, const Uniform &uniform)
        {
            ...
            Vector3f diffuse = attribute.light_color * sampling(uniform.albedo, fi.I_UV) * attribute.light_intensity * std::max(0.f, Utils::dot_product_3D(fi.IWS_NORMAL, -1 * attribute.world_light_dir));
